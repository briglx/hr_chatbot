...
class TestEmbeddingService:
    ...
    def make_embedding_response(self, vectors: list[list[float]], total_tokens: int = 10):
        """Build a fake OpenAI embeddings response."""
        response = MagicMock()
        response.data = [
            MagicMock(embedding=vector, index=i)
    ...
    async def test_embed_calls_correct_model(self):
        with patch("app.services.embedding_service.AsyncAzureOpenAI") as mock_openai:
            create_mock = AsyncMock(return_value=self.make_embedding_response([self.FAKE_VECTOR]))
    ...
    async def test_embed_batch_preserves_order(self):
        ...
        with patch("app.services.embedding_service.AsyncAzureOpenAI") as mock_openai:
            mock_openai.return_value.embeddings.create = AsyncMock(return_value=response)
            svc = EmbeddingService()
...
class TestRetrievalService:
    ...
    def make_row(self, id=1, content="You get 20 vacation days per year.", source="hr-policy.pdf", score=0.92, metadata=None):
    ...
    def mock_session(self):
        ...
        with patch("app.services.retrieval_service.create_async_engine"), \
             patch("app.services.retrieval_service.sessionmaker", return_value=session_factory):
            yield session
    ...
    async def test_search_returns_document_chunks(self, mock_session):
        ...
        mock_session.execute = AsyncMock(return_value=MagicMock(fetchall=MagicMock(return_value=rows)))
    ...
    async def test_close_disposes_engine(self):
        with patch("app.services.retrieval_service.create_async_engine") as mock_engine, \
